...
function buildOpenCodeLauncherCmd() {
  return [
    "@echo off",
    "setlocal",
    "set \"SYNC_PATH=%USERPROFILE%\\.config\\opencode\\9router-opencode-sync.ps1\"",
    "if exist \"%SYNC_PATH%\" powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File \"%SYNC_PATH%\" -Once >nul 2>nul",
    "set \"REAL_OPENCODE=\"",
    "for /f \"delims=\" %%P in ('where opencode 2^>nul') do (",
    "  if /I not \"%%~fP\"==\"%~f0\" (",
    "    set \"REAL_OPENCODE=%%~fP\"",
    "    goto found_opencode",
    "  )",
    ")",
    ":found_opencode",
    "if not defined REAL_OPENCODE (",
    "  echo OpenCode asli tidak ditemukan di PATH.",
    "  exit /b 1",
    ")",
    "endlocal & call \"%REAL_OPENCODE%\" %*",
    "",
  ].join("\n");
}
...
function buildBat(config) {
  ...
  if (toolId === "opencode") {
    return buildPlainBat([
      ...
      "  set \"CONFIG_URL=https://route9.nurset-studio.web.id/api/v1/model-integration/opencode.json?baseUrl=https%%3A%%2F%%2Froute9.nurset-studio.web.id%%2Fv1\"",
      ")",
      ...
      ...writeBatchFileBlock("LAUNCHER_PATH", buildOpenCodeLauncherCmd()),
      "powershell.exe -NoProfile -ExecutionPolicy Bypass -Command \"$dir='%LAUNCHER_DIR%'; $path=[Environment]::GetEnvironmentVariable('Path','User'); if ([string]::IsNullOrWhiteSpace($path)) { $new=$dir } elseif (($path -split ';') -notcontains $dir) { $new=$dir+';'+$path } else { $new=$path }; [Environment]::SetEnvironmentVariable('Path',$new,'User')\"",
      `setx OPENAI_API_KEY "${escapeBat(apiKey)}" >nul`,
      "echo OpenCode config updated at %CONFIG_PATH%",
      "echo Sync lama sudah dihapus. OpenCode akan sync sekali setiap command opencode dibuka dari terminal baru.",
      "echo Tutup dan buka ulang terminal agar PATH launcher OpenCode terbaca.",
      "goto done",
